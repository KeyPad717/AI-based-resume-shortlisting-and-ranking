...
# ---------------------------------------------------------------------------
# Phase 10: lazy LLM dispatcher (cache + JSONL log + concurrency + cost cap).
# Built on first use so pipeline import stays side-effect-free and the
# RAG-disabled path never pays for cache/log setup. Reused by the three LLM
# call sites: call_llm_extraction, SkillNormalizer._classify_canonical and
# EvidenceVerifier._call_llm_batch.
# ---------------------------------------------------------------------------
_llm_dispatcher = None


def _build_llm_dispatcher(llm_client):
...
def _get_llm_dispatcher(llm_client=None):
    """Return the process-wide dispatcher for the REAL client, or a fresh
    per-client dispatcher for any other (e.g. injected/fake) client.

    The singleton shares one cache/log/semaphore/budget across every LLM call
    site in a job, which is what makes the per-job cost ceiling actually a
    per-JOB ceiling. A non-default client (test fakes, alternate backends) gets
    a fresh dispatcher so it never reuses the production singleton's budget.
    """
    global _llm_dispatcher
    if llm_client is None or llm_client is client:
        if _llm_dispatcher is not None:
            return _llm_dispatcher
        _llm_dispatcher = _build_llm_dispatcher(client)
        return _llm_dispatcher
    return _build_llm_dispatcher(llm_client)
...
def call_llm_extraction(text, is_jd=False, retries=2):
    ...
    dispatcher = _get_llm_dispatcher()
    for attempt in range(retries):
        try:
            # Phase 10: dispatch through the content-addressed cache + JSONL
            # logger + semaphore + cost budget. Identical prompt -> cache hit.
            payload = dispatcher.call(
                op="extraction",
                prompt=prompt,
                model=EXTRACTION_MODEL,
                cache_version=EXTRACTION_PROMPT_VERSION,
                cache_key_supplement="jd" if is_jd else "resume",
            )
            if payload is None:
                raise RuntimeError("LLM call returned no parseable payload")
            return payload
...
def score_candidate_rag(jd_text, resume_text, r_struct, jd_struct, r_path,
    ...
    # Phase 10: all LLM calls in a job share ONE dispatcher (content cache,
    # JSONL log, concurrency semaphore, per-job cost ceiling).
    dispatcher = _get_llm_dispatcher(llm_client)

    normalizer = SkillNormalizer(ontology_store, embedder, llm_client, dispatcher=dispatcher)
